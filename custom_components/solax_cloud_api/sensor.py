...
# --- SolaxFieldSensor class is unchanged, so it is omitted for brevity ---
class SolaxFieldSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, serial, field, unique_id, unit=None, system_slug=None):
        ...
        self.entity_id = f"sensor.{suggested_id}"
    ...
    def state(self):
        ...
        if not inv or not isinstance(inv, dict): return 0 if self._field in NUMERIC_FIELDS else None
        if inv.get("error"): return None
        val = inv.get(self._field)
        if val is None: return None
        if self._field in MAPPED_FIELDS: return str(val)
        if self._field in NUMERIC_FIELDS: return val
        return val
    @property
    def device_class(self):
        if self._field in NUMERIC_FIELDS:
            field_type = NUMERIC_FIELDS[self._field][1]
            if field_type == "energy": return SensorDeviceClass.ENERGY
            elif field_type == "power": return SensorDeviceClass.POWER
            elif field_type == "battery": return SensorDeviceClass.BATTERY
        return None
    @property
    def state_class(self):
        if self._field in NUMERIC_FIELDS:
            field_type = NUMERIC_FIELDS[self._field][1]
            if field_type == "energy": return SensorStateClass.TOTAL_INCREASING
            else: return SensorStateClass.MEASUREMENT
        return None
    @property
    def available(self):
        ...
        if self._field in MAPPED_FIELDS or self._field in NUMERIC_FIELDS: return data.get(self._field) is not None
        return True
    @property
    def device_info(self):
        inv = self.coordinator.data.get(self._serial)
        if not inv or not isinstance(inv, dict):
            return {"identifiers": {(DOMAIN, self._serial)}, "name": f"Solax Inverter {self._serial}", "manufacturer": "Solax", "model": "Unknown"}
        inverter_sn = inv.get("inverterSN") or self._serial
        model = "Unknown"
        inverter_type_val = inv.get("inverterType")
        if inverter_type_val is not None: model = str(inverter_type_val)
        return {"identifiers": {(DOMAIN, inverter_sn)}, "name": f"Solax Inverter {inverter_sn}", "manufacturer": "Solax", "model": model, "serial_number": inverter_sn}
    @property
    def extra_state_attributes(self):
        attrs = {}
        inv = self.coordinator.data.get(self._serial)
        if not inv or not isinstance(inv, dict): return attrs
...
# --- SolaxInverterEfficiencySensor class is unchanged ---
class SolaxInverterEfficiencySensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, serial, unique_id, system_slug=None):
        ...
        self._attr_state_class = SensorStateClass.MEASUREMENT
        self._attr_suggested_display_precision = 1
        if system_slug: self._attr_suggested_object_id = f"{system_slug}_inverter_efficiency_{serial}".lower()
    @property
    def state(self):
        inv = self.coordinator.data.get(self._serial)
        if not inv or not isinstance(inv, dict): return 0
        dc_total = sum(inv.get(f"powerdc{i}", 0) for i in range(1, 5))
        ac = inv.get("acpower") or 0
        if dc_total > 0: return round((ac / dc_total) * 100, 1)
        return 0
    @property
    ...
    def available(self):
        inv = self.coordinator.data.get(self._serial)
        if not inv or not isinstance(inv, dict): return False
    ...
    def device_info(self):
        ...
        model = "Unknown"
        if isinstance(inv, dict) and inv.get("inverterType") is not None: model = str(inv.get("inverterType"))
        return {"identifiers": {(DOMAIN, inverter_sn)}, "name": f"Solax Inverter {inverter_sn}", "manufacturer": "Solax", "model": model, "serial_number": inverter_sn}

# --- SolaxComputedSensor class is unchanged ---
class SolaxComputedSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, serial, metric, unique_id, system_slug=None):
        ...
        self._attr_state_class = SensorStateClass.MEASUREMENT
        self._attr_device_class = SensorDeviceClass.POWER
        if not system_slug: raise ValueError("System slug must be provided for SolaxComputedSensor")
    ...
    def state(self):
        ...
        for i in range(1, 5):
            ...
            if field in inv:
                ...
                power = inv.get(field)
                if power is not None: total += power
        if not has_any_dc_field:
            if inv.get("acpower") is None: return 0
            return None
        return total
    @property
    def unit_of_measurement(self): return "W"

# --- SolaxSystemTotalSensor has the CRITICAL FIX ---
class SolaxSystemTotalSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, inverters, metric):
        ...
        # THIS IS THE FIX: Re-introducing the metric_map to ensure the unique_id remains stable and does not change.
        metric_map = {
            "ac_total": "ac_power",
            "dc_total": "dc_power", 
        ...
        # The unique_id now uses the STABLE key from the metric_map, just like it did originally.
        # This prevents the "entity no longer provided" error.
    ...
    def device_info(self):
        ...
        return {"identifiers": {(DOMAIN, f"system_totals_{system_name.replace(' ', '_').lower()}")}, "name": f"{system_name} ({active_inverters}/{total_inverters} Inverters)", "manufacturer": "Solax", "model": f"Multi-Inverter System ({active_inverters} active, {total_inverters} total)"}
        ...
        for sn in self._inverters:
            ...
            if inv and isinstance(inv, dict):
                if self._metric == "ac_total" and inv.get("acpower") is not None: return True
                    ...
                    if inv.get("acpower") is not None and any(inv.get(f"powerdc{i}") is not None for i in range(1, 5)): return True
        return False
    ...
    def state(self):
        if self._metric == "systemEfficiency":
            ...
            for sn in self._inverters:
                ...
                total_ac += ac
                total_dc += dc
            if total_dc > 0: return round((total_ac / total_dc) * 100, 1)
            return 0
        total = 0
        for sn in self._inverters:
            inv = self.coordinator.data.get(sn)
            ...
            if self._metric in field_map:
                ...
                for i in range(1, 5):
                    ...
                    if power is not None: dc_total += power
                total += dc_total
        return total

    @property
    def state_class(self):
        if self._metric in ("yieldtoday_total", "yieldtotal_total"): return SensorStateClass.TOTAL_INCREASING
        return SensorStateClass.MEASUREMENT

    @property
    def device_class(self):
        if self._metric in ("yieldtoday_total", "yieldtotal_total"): return SensorDeviceClass.ENERGY
        if self._metric == "systemEfficiency": return None
        return SensorDeviceClass.POWER

    @property
    def unit_of_measurement(self):
        if self._metric == "systemEfficiency": return "%"
        if self._metric in ("ac_total", "dc_total"): return "W"
        return "kWh"

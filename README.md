...
2. **API Token** - In SolaX Cloud, in the top-right or the screen, select **API** from the menu:
...
4. Enter your configuration:
   - **API Region**: Select **Global** for the standard SolaX Cloud API, **India** for accounts migrated to the dedicated India API domain, or **North America** for accounts on the dedicated NA API domain
...
4. Change **API Region** if the SolaX account has moved between the Global, India, and North America API services
...
- **🌏 API Region**: Existing installations default to **Global**. India- and North America-region users can select **India** or **North America** during setup or later under **Configure**.
